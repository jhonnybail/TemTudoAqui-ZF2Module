...
class Module
{
    ...
    public function onBootstrap(MvcEvent $e)
    {
        ...
        }elseif(!file_exists($sys['pathSession'].$e->getRequest()->getCookie()->session.".cok")){
            $file = File::CreateFileByString("[]");
            FileReference::Save($file, $sys['pathSession'], $e->getRequest()->getCookie()->session, 'cok');
        }
    ...
    public function loadConfiguration(MvcEvent $e)
    {
        ...
        try{
            ...
            if($controller instanceof \TemTudoAqui\Common\Controller && false){

                if(!empty($idUserAuth) && !preg_match('!list!', $actionName) && $controller->getResourceName() != 'logger'){
                        ...
                        $resources              = $daoResource->find($resource);
                        if($resources->count() > 0)
                            $logger->resource   = $resources[0];
    ...
    public function getServiceConfig(){
        return array(
            'factories' => array(
                ...
                'TTA-AuthenticationService' => function($service){
                    $auth = new AuthenticationService;
                    $sessionStorage = new Session("tta_admin");
